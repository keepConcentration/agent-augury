...
## 6c. M5 — 원 요청을 P2·P5 프롬프트에 되울린다 (2026-09-23)

**관측.** 사용자가 "미비한 점, 만들어야 할 점, 개선할 점, 추가하면 좋을 기능"을
리뷰해 MD로 달라고 했다. P2 가 낸 안은 두 줄 다 `ASSIGN ...: Explore and
document ...` 였고, 나온 건 아키텍처 설명서였다. 네 항목 중 **하나도** 안 들어갔다.

**왜 아무도 못 잡았나.** P1~P5 어디에도 산출물을 원 요청과 대조하는 자리가 없다.

| 단계 | 프롬프트가 시키는 것 | 원 요청 언급 |
|------|---------------------|-------------|
| P2 | "Negotiate a split of sub-questions" | 없음 (무엇의 sub-question 인지 안 말함) |
| P4 | "Review teammates' submissions for ... omissions" | 없음 (제출물끼리 비교) |
| P5 | "Post it ... starting with `FINAL:`" | 없음 (존재만 요구) |

즉 프로토콜은 **내부 정합성 검사기**다. 팀이 서로 말이 맞는지는 본다. 팀이 통째로
엉뚱한 걸 만드는지는 안 본다. 그래서 P4 리뷰는 규정대로 동작했다 — "omission" 을
찾았고, 찾은 게 *"agent-2 가 스레드에 글을 안 올림"* 이었다. 진짜 omission 은
*"아무도 리뷰를 안 만듦"* 이었지만 그건 P4 의 관심사가 아니다.

**결정.** 요청 원문을 `Session.task` 에 붙들고 **P2 와 P5 프롬프트에만** 되울린다.

- **P2** — 분업이 무엇을 만들지 고정하는 자리. "요청 전체를 덮어라, 항목마다
  `ASSIGN` 이 있어야 한다" + 원문.
- **P5** — 완료를 선언하는 자리. "초안을 원 요청과 대조해라, 빠진 항목이 있으면
  `FINAL:` 이 아니라 `REJECT:` 다" + 원문.
- **P1/P3/P4 는 제외** — 이 셋은 어긋남을 *물려받지* 만들지 않는다. 원문은
  이미 턴 시작에 각 에이전트 대화에 들어가 있어서, 세 번 더 붙이면 토큰만 샌다.

원문은 1200자에서 자른다(`_TASK_ECHO_MAX`). 프롬프트의 사본은 대조용이면 충분하고
전문은 대화에 이미 있다.

**체크포인트에 싣는다.** `meta.json` 의 `task`. 재개 때 잃으면 두 검사가 조용히
무장 해제된다. 설정의 `task` 는 기본값이고 체크포인트 값이 이긴다.

**게이트가 아니다.** ASSIGN 과 같이 자문(advisory) 이다. 프롬프트 문구일 뿐이라
잘못된 요청이나 빈 `task` 가 세션을 막지 않는다. 모델이 무시하면 그만이다 — 이건
모델 실수를 *줄이는* 장치지 *막는* 장치가 아니다. 막으려면 P5 게이트에 기계적
체크리스트가 필요한데, 항목 추출이 다시 모델 일이라 P0 범위 밖으로 둔다.
