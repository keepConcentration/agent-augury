...
**후속 조치.** 검증 결과 39스텝·16메시지 → **8스텝·0메시지**가 됐다. 다만 흉내와
함께 **교차 검증도 사라졌다** — 네 에이전트가 각자 답을 쓰고 아무도 읽지 않는다.
여기에 `light` 한 라운드를 붙이는 안은 `FOLLOWUP_TURN_PROTOCOL_DESIGN.md` 로
분리했다. 그 설계를 착수하면 **본 §8.3 의 프롬프트 블록이 정반대 지시가 되므로**
같이 손봐야 한다.
