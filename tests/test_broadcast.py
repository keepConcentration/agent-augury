...
def test_wizard_flow_preserves_attention_on_new_session(tmp_path):
    """Silent reuse / --new-session must not drop hand-edited attention:."""
    from agent_augury.cli import _run_wizard_flow
    ...
    assert saved["attention"] == {"enabled": True}
    assert saved["task"] == "keep-me"
    assert saved["agents"][0]["id"] == "agent-1"  # from model_config
    assert len(calls) == 1


def test_preserve_session_extras_unit():
    from agent_augury.cli import _preserve_session_extras

    fresh = {"max_steps": 10, "agents": [{"id": "a"}], "protocol": {"mode": "full"}}
    existing = {
        "max_steps": 1,
        "agents": [{"id": "old"}],
        "attention": {"enabled": True},
        "task": "hi",
        "_attention_normalized": {"enabled": True},
    }
    merged = _preserve_session_extras(existing, fresh)
    assert merged["max_steps"] == 10
    assert merged["agents"][0]["id"] == "a"
    assert merged["attention"] == {"enabled": True}
    assert merged["task"] == "hi"
    assert "_attention_normalized" not in merged
