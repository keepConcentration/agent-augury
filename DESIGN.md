...
#### 3.5.4 동시성 모델 (v0.1a 기본값 고정)

**결정:** 단일 asyncio 이벤트 루프, 프로세스 내 단일 서버 태스크.

- **asyncio 단일 루프**: N개 에이전트 루프 + 서버가 한 이벤트 루프에서 협력 스케줄링. (A 모델이므로 별도 워처 태스크 없음, §3.5.2)
...
이 결정은 v0.1a에서 동시성·잠금 논쟁을 미리 제거하기 위한 것이며, 에이전트 규모가 커지면 프로세스 격리로 재검토한다.
