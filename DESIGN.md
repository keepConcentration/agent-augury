...
네 에이전트(수는 사용자 구성)가 고정 프로토콜을 돈다. 스레드는 세션 기동 시 미리 열리고, 페이즈 전이는 **게이트**가 판정한다 — 게이트가 요구하는 **진입 신호**(P2 `PROPOSE:`, P5 `FINAL:`)가 스레드에 있고 **모든 에이전트가 명시적으로 승인**해야 다음 페이즈로 간다. 고정 어셈블러 역할은 없다(누가 제안하고 누가 조립할지는 논의로 정해진다).
...
2. **P2 분할** — 계획 스레드에 발견 사항 풀링 → 하위 질문 분할 협상 → `PROPOSE:` 로 분할안 게시 → **전원 승인**까지 수정.
3. **P3 실행** — 각자 자기 몫 수행. 발견 즉시 워크로그에 공유(중간 발견/모순/장애/포기된 접근).
4. **P4 교차검토** — 각자 결과 스레드에 근거와 함께 방송. 검토자들이 사실 충돌·근거 부족·누락 관찰 지적.
5. **P5 제출** — 누구든 승인된 결과로 최종 답을 조립해 `FINAL:` 로 방송 → **전원 승인** → 제출. `FINAL:` 초안이 없으면 게이트는 열리지 않는다.
...
교차검토(P4), 최종 조립·제출(P5) 등 원본의 전체 프로토콜 구현 완료.
